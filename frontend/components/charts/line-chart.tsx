...
export function LineChartComponent({ data, viewType, selectedCategories, categoryColors }: LineChartProps) {
  ...
  useEffect(() => {
    ...
    if (viewType === "total") {
      ...
      const categoryValues = data.flatMap((item) => selectedCategories.map((cat) => item.categories[cat] || 0))
