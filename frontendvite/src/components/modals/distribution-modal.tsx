...
import { X, Check, Users, Percent, Plus, Lock, Unlock, Baby, Trash2, Euro, SwitchCamera } from "lucide-react"
...
export function DistributionModal({
  ...
  const totalExpenseAmount = Number.parseFloat(String(expenseAmount ?? "0").replace("€", "").replace(",", ".")) || 0
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (modalRef.current && !modalRef.current.contains(event.target as Node) && animation === "entered") {
        handleClose()
      }
    }
  ...
  const handleSave = () => {
    ...
    const finalParticipants = participants.map((p) => {
      if (allocationMode === "percentage") {
        ...
        const totalAllocated = participants.reduce((sum, part) => sum + (part.amount || 0), 0)
  ...
  const handlePercentageChange = (id: string, value: number) => {
    ...
    if (unlocked.length > 0) {
      ...
      if (total !== 100) {
        ...
        // Update amount as well
        participantToUpdate.amount = (participantToUpdate.percentage / 100) * totalExpenseAmount
      }
    }
    ...
    if (Math.abs(total - 100) > 0.1) {
      ...
      if (adjustableParticipant) {
        ...
        adjustableParticipant.percentage = Math.round(adjustableParticipant.percentage * 10) / 10
        // Update amount as well
        adjustableParticipant.amount = (adjustableParticipant.percentage / 100) * totalExpenseAmount
        ...
        participantToUpdate.percentage = Math.round(participantToUpdate.percentage * 10) / 10
        // Update amount as well
        participantToUpdate.amount = (participantToUpdate.percentage / 100) * totalExpenseAmount
      }
    }
  ...
  const handleEvenDistribution = () => {
    ...
    if (allocationMode === "percentage") {
      ...
      if (Math.abs(total - 100) > 0.1 && updatedParticipants.length > 0) {
        ...
        updatedParticipants[0].percentage = Math.round(updatedParticipants[0].percentage * 10) / 10
        updatedParticipants[0].amount =
          Math.round((updatedParticipants[0].percentage / 100) * totalExpenseAmount * 100) / 100
      }
      ...
      if (Math.abs(totalAmount - totalExpenseAmount) > 0.01 && updatedParticipants.length > 0) {
        updatedParticipants[0].amount = (updatedParticipants[0].amount || 0) + (totalExpenseAmount - totalAmount)
        updatedParticipants[0].amount = Math.round((updatedParticipants[0].amount || 0) * 100) / 100
      }
  ...
  const toggleLock = (id: string) => {
    setParticipants(participants.map((p) => (p.id === id ? { ...p, locked: !p.locked } : { ...p })))
  }
  ...
  const addParticipant = () => {
    ...
    const newName = isChildExpense && newId === "user3" ? "Kind" : `Partner ${participants.length + 1}`
    ...
    if (Math.abs(totalPercentage - 100) > 0.1 || Math.abs(totalAmount - totalExpenseAmount) > 0.01) {
      // Find an unlocked participant to adjust
      ...
      if (adjustableParticipant) {
        ...
        if (Math.abs(totalAmount - totalExpenseAmount) > 0.01) {
          adjustableParticipant.amount = (adjustableParticipant.amount || 0) + (totalExpenseAmount - totalAmount)
          adjustableParticipant.amount = Math.round((adjustableParticipant.amount || 0) * 100) / 100
        }
      }
    }
    ...
    if (unlockedParticipants.length > 0) {
      if (allocationMode === "percentage") {
        ...
        remainingParticipants.forEach((p) => {
          if (!p.locked) {
            const ratio = unlockedTotal > 0 ? p.percentage / unlockedTotal : 1 / unlockedParticipants.length
        ...
        const unlockedTotal = unlockedParticipants.reduce((sum, p) => sum + (p.amount || 0), 0)

        // Distribute the removed participant's amount among unlocked participants
        remainingParticipants.forEach((p) => {
          if (!p.locked) {
            const ratio = unlockedTotal > 0 ? (p.amount || 0) / unlockedTotal : 1 / unlockedParticipants.length
            p.amount = (p.amount || 0) + amountToRedistribute * ratio
            p.amount = Math.round((p.amount || 0) * 100) / 100
            // Update percentage as well
            const totalAmount = remainingParticipants.reduce((sum, part) => sum + (part.amount || 0), 0)
            p.percentage = totalAmount > 0 ? Math.round(((p.amount || 0) / totalAmount) * 1000) / 10 : 0
          }
        })
      }
    } else {
      ...
      if (remainingParticipants.length > 0) {
        ...
        if (allocationMode === "percentage") {
          ...
          remainingParticipants[0].percentage = Math.round(remainingParticipants[0].percentage * 10) / 10
          remainingParticipants[0].amount =
            Math.round((remainingParticipants[0].percentage / 100) * totalExpenseAmount * 100) / 100
        } else {
          remainingParticipants[0].amount = (remainingParticipants[0].amount || 0) + amountToRedistribute
          remainingParticipants[0].amount = Math.round((remainingParticipants[0].amount || 0) * 100) / 100
  ...
  const toggleAllocationMode = () => {
    ...
    const updatedParticipants = participants.map((p) => {
      if (allocationMode === "percentage") {
        ...
        return {
          ...p,
          percentage: totalAllocated > 0 ? Math.round(((p.amount || 0) / totalAllocated) * 1000) / 10 : 0,
        }
      }
    })
        ...
        <div className="p-4 pb-safe overflow-y-auto max-h-[calc(100%-56px)] flex flex-col">
          ...
          <div className="mb-4">
            <div className="flex justify-between items-center">
              <h4 className="text-sm font-medium text-gray-700">Verteilungsmethode</h4>
              <button
                onClick={(e) => {
                  e.stopPropagation()
              ...
              >
                ...
                <span className="text-xs">
                  {allocationMode === "percentage" ? "Zu Beträgen wechseln" : "Zu Prozenten wechseln"}
                </span>
              </button>
            </div>
            ...
            <div className="mt-2 p-2 bg-blue-50 rounded-lg text-xs text-blue-700">
              {allocationMode === "percentage" ? (
                <div className="flex items-center">
                  ...
                  <span>
                    Prozentuale Verteilung: Jeder Teilnehmer erhält einen prozentualen Anteil der Gesamtausgabe.
                  </span>
                </div>
              ) : (
                <div className="flex items-center">
                  <Euro className="h-4 w-4 mr-1 flex-shrink-0" />
                  <span>Betragsverteilung: Geben Sie für jeden Teilnehmer einen konkreten Betrag an.</span>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="mb-4">
            <div className="flex justify-between items-center mb-3">
              <h4 className="text-sm font-medium text-gray-700">
                {allocationMode === "percentage" ? "Prozentuale Verteilung" : "Betragsverteilung"}
              </h4>
              ...
              {participants.map((participant) => (
                ...
                >
                  <div className="flex justify-between items-center mb-2">
                    <div className="flex items-center">
                      <div className={`${participant.locked ? "bg-blue-100" : "bg-blue-50"} p-2 rounded-full mr-2`}>
                    ...
                    <div className="flex items-center space-x-2">
                        ...
                        {allocationMode === "percentage" ? (
                            ...
                            <input
                              ...
                              onChange={(e) => {
                                ...
                                if (!participant.locked) {
                                  handlePercentageChange(participant.id, Number.parseFloat(e.target.value) || 0)
                                }
                              }}
                          ...
                          <>
                            ...
                            <input
                              ...
                              min="0"
                              step="0.01"
                              value={(participant.amount || 0).toFixed(2)}
                              onChange={(e) => {
                                e.stopPropagation()
                                if (!participant.locked) {
                                  handleAmountChange(participant.id, Number.parseFloat(e.target.value) || 0)
                                }
                              }}
                      ...
                      <button
                        ...
                        aria-label={participant.locked ? "Entsperren" : "Sperren"}
                      >
                        {participant.locked ? <Lock className="h-3.5 w-3.5" /> : <Unlock className="h-3.5 w-3.5" />}
                      </button>
                    ...
                    <div className="relative pt-1">
                      <input
                        ...
                        onChange={(e) => {
                          e.stopPropagation()
                          if (!participant.locked) {
                            handlePercentageChange(participant.id, Number.parseFloat(e.target.value))
                          }
                        }}
                        className={`w-full h-2 rounded-lg appearance-none cursor-pointer relative z-10 ${
                          participant.locked ? "opacity-70 cursor-not-allowed" : ""
                        }`}
                        style={{
                  ...
                  {allocationMode === "percentage" && (
                    <div className="mt-2 text-xs text-gray-500 text-right">
                      ≈ €{((participant.percentage / 100) * totalExpenseAmount).toFixed(2)}
                    </div>
                  )}

                  {/* Percentage display in amount mode */}
                  {allocationMode === "amount" && (
                    <div className="mt-2 text-xs text-gray-500 text-right">≈ {participant.percentage.toFixed(1)}%</div>
                  )}
                </div>
              ))}
            </div>
          ...
          <div className="mb-4 bg-blue-50 p-3 rounded-xl">
            {allocationMode === "percentage" ? (
              <div className="flex justify-between items-center">
                ...
                <span
                  className={`text-sm font-bold ${
                    Math.abs(participants.reduce((sum, p) => sum + p.percentage, 0) - 100) > 0.1
                      ? "text-red-600"
                      : "text-blue-700"
                  }`}
                >
              ...
              <div className="space-y-2">
                <div className="flex justify-between items-center">
                  <span className="text-sm font-medium text-blue-700">Gesamtausgabe:</span>
                  <span className="text-sm font-bold text-blue-700">€{totalExpenseAmount.toFixed(2)}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-sm font-medium text-blue-700">Zugewiesen:</span>
                  <span className={`text-sm font-bold ${isOverAllocated ? "text-red-600" : "text-blue-700"}`}>
                    €{totalAllocatedAmount.toFixed(2)}
                  </span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-sm font-medium text-blue-700">Verbleibend:</span>
                  <span className={`text-sm font-bold ${isOverAllocated ? "text-red-600" : "text-green-600"}`}>
                    {isOverAllocated ? "-" : ""}€{Math.abs(remainingAmount).toFixed(2)}
                  </span>
                </div>
              </div>
            )}
          ...
          {allocationMode === "amount" && isOverAllocated && (
            <div className="mb-4 p-2 bg-red-50 border border-red-200 rounded-lg text-xs text-red-600">
              Warnung: Die zugewiesenen Beträge übersteigen die Gesamtausgabe. Bitte passen Sie die Beträge an.
            </div>
          )}
