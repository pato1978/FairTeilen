      ...
      <div className="p-3">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center">
            {statusInfo.icon}
            <span className={`ml-1.5 text-sm ${statusInfo.textColor}`}>{statusInfo.text}</span>
          </div>
        ...
        {needsClarification && month.message && (
          <div className="mt-2 text-xs bg-amber-100 text-amber-700 p-1.5 rounded">{month.message}</div>
        )}

        {isFuture && (
          <div className="mt-2 text-xs bg-gray-100 text-gray-500 p-1.5 rounded">Erst nach Monatsende verfügbar</div>
        )}

        {isCompleted && <div className="mt-2 text-xs bg-gray-100 text-gray-500 p-1.5 rounded">Monat abgeschlossen</div>}
      </div>
    </div>
  )
}
